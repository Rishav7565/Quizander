...
#include<stdlib.h>
#define TRUE 1
int multi_poll(int,int);
int multi_poll_bas(int);
int multi_poll_inter(int);
int multi_poll_final(int);
int main()
{
	int a1,a2;
	printf("lets start");
...
int multi_poll_bas(int w)
{
	int a[7],i,j,temp,bm1=0;
	srand(time(NULL));
	for(i=0;i<7;i++)
	{
		temp=rand()%7;
...
int multi_poll_inter(int x)
{
	int b[10],bm2=0,i,j,temp;
	srand(time(NULL));
	for(i=0;i<=9;i++)
	{
		temp=rand()%10;
...
return (bm2);    
  ...
  int multi_poll_final(int y)
  {
  		int c[12],bm3=0,i,j,temp;
	srand(time(NULL));
	for(i=0;i<=12;i++)
	{
		temp=rand()%12;
